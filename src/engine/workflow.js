...
function generateOpenCodeScanPrompt(repoPath) {
  ...
  const prompt = [
    ...
    'APIMAP SCAN — PURPOSE',
    '═══════════════════════════════════════════════════════════',
    'You are generating scan cache files for the APIFlow dashboard.',
    ...
    '  .apimap/api_knowledge.json  ← PRIMARY OUTPUT (required)',
    '  .apimap/metadata.json       ← AUDIT TRAIL (required)',
    '',
    'graph.json and scan_state.json are derived automatically by the',
    'APIFlow server on first boot — you do NOT need to generate them.',
    '',
    ...
    '',
    '3) Create `.apimap/metadata.json` — a JSON array, one entry per route:',
    ...
    '',
    '4) Create `.apimap/api_knowledge.json` — a JSON array, one wrapper object per route:',
